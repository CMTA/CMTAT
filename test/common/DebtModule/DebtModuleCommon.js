...
function DebtModuleCommon () {
  context('Debt Module test', function () {
    ...
    it('testCanSetAndGetDebtCorrectly', async function () {
      ...
      await expect(this.logs).to.emit(this.cmtat, 'DebtLogEvent')
    ...
    it('testCanSetAndGetDebtInstrumentCorrectly', async function () {
      this.logs = await this.cmtat
        .connect(this.admin)
        .setDebtInstrument(debtBase.debtInstrument)
      await expect(this.logs).to.emit(this.cmtat, 'DebtInstrumentLogEvent')
      const debt = await this.cmtat.debt()
      // debt instrument
      expect(debt.debtInstrument.interestRate).to.equal(
        debtBase.debtInstrument.interestRate
    ...
    it('testCannotBeSetDebtWithoutDebtRole', async function () {
      ...
      // Without reason
      await expect(this.cmtat.connect(this.address2).setDebt(debtBase))
        .to.be.revertedWithCustomError(
          this.cmtat,
          'AccessControlUnauthorizedAccount'
        )
    ...
    it('testCanSetAndGetCreditEventsCorrectly', async function () {
      ...
      await expect(this.logs).to.emit(this.cmtat, 'CreditEventsLogEvent')
