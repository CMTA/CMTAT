...
contract('MintModule', function ([_, owner, address1, address2, address3, fakeRuleEngine]) {
  ...
  context('Minting', function () {
    it('can be minted as the owner', async function () {
      ...
      ({ logs: this.logs1 } = await this.cmtat.mint(address1, 20, {from: owner}));
      ...
      ({ logs: this.logs2 } = await this.cmtat.mint(address2, 50, {from: owner}));
    ...
    it('can be minted by anyone with minter role', async function () {
      ...
      // Issue 20 and check balances and total supply
      ({ logs: this.logs1 } = await this.cmtat.mint(address1, 20, {from: address1}));
      (await this.cmtat.balanceOf(address1)).should.be.bignumber.equal('20');
      (await this.cmtat.totalSupply()).should.be.bignumber.equal('20');
    }); 

    it('emits a Transfer event', function () {
      expectEvent.inLogs(this.logs1, 'Transfer', { from: ZERO_ADDRESS, to: address1, value: '20' });
      expectEvent.inLogs(this.logs2, 'Transfer', { from: ZERO_ADDRESS, to: address2, value: '50' });
    });

    it('emits a Mint event', function () {
      expectEvent.inLogs(this.logs1, 'Mint', { beneficiary: address1, amount: '20' });
      expectEvent.inLogs(this.logs2, 'Mint', { beneficiary: address2, amount: '50' });
    });
