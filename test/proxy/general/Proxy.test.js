...
contract(
  ...
  function ([_, admin, attacker]) {
    beforeEach(async function () {
      this.CMTAT_PROXY = await deployProxy(CMTAT1, [true, admin, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch', ZERO_ADDRESS], { initializer: 'initialize', constructorArgs: [_, true, admin, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch', ZERO_ADDRESS] })
    ...
    context('Attacker', function () {
      ...
      it('testCannotBeTakenControlByAttacker1', async function () {
        await expectRevert(
          this.implementationContract.initialize(false, attacker, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch', ZERO_ADDRESS, { from: attacker }),
      ...
      it('testCannotBeTakenControlByAttacker2', async function () {
        await expectRevert(
          this.implementationContract.initialize(true, attacker, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch', ZERO_ADDRESS, { from: attacker }),
          'Initializable: contract is already initialized'
        )
        await expectRevert(
          this.implementationContract.kill({ from: attacker }),
