...
contract(
  ...
  function ([_, admin, attacker]) {
    beforeEach(async function () {
      this.CMTAT_PROXY = await deployProxy(CMTAT1, [admin, _, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch'], { initializer: 'initialize', constructorArgs: [] })
    ...
    context('Attacker', function () {
      ...
      it('testCannotBeTakenControlByAttacker1', async function () {
        await expectRevert(
          this.implementationContract.initialize(attacker, _, 'CMTA Token', 'CMTAT', 'CMTAT_ISIN', 'https://cmta.ch', { from: attacker }),
