...
| 3 | Operator identity propagation in burn/mint/cross-chain RuleEngine paths | Valid policy-consistency gap for operator-aware RuleEngine policies | Fixed | v3.0.0, v3.1.0, v3.2.0 (only when RuleEngine is enabled and rules target spender/operator) | `10e87ff`, `7a7c975` | Medium | Low | Core issue was policy expressiveness/consistency, not direct unauthorized access; impact is limited to CMTAT deployments using RuleEngine rules that evaluate spender/operator identity. Affected functions are role-protected (minter/burner/bridge roles), reducing exploitability from unprivileged actors. |
